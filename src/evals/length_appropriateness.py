...
def analyze_topic_complexity(topic_docs: List[str]) -> Dict:
    ...
    for sentence in sentences:
        ...
        if len(words_in_sentence) > 1:
            non_initial_caps += sum(
                1 for w in words_in_sentence[1:] if w and w[0].isupper()
            )

    information_density = (
        vocabulary_diversity + complex_ratio + (non_initial_caps / max(1, word_count))
    ) / 3
    ...
    if overall_complexity < config.get(
        "length_appropriateness.overall_complexity_simple"
    ):
        complexity_category = "simple"
    elif overall_complexity < config.get(
        "length_appropriateness.overall_complexity_moderate"
    ):
...
def calculate_optimal_turn_range(complexity_analysis: Dict) -> Dict:
    ...
    ranges = {
        "simple": {
            "min": config.get("length_appropriateness.simple_turn_min"),
            "max": config.get("length_appropriateness.simple_turn_threshold"),
            "ideal": config.get("length_appropriateness.simple_turn_ideal"),
        },
        "moderate": {
            "min": config.get("length_appropriateness.moderate_turn_min"),
            "max": config.get("length_appropriateness.moderate_turn_threshold"),
            "ideal": config.get("length_appropriateness.moderate_turn_ideal"),
        },
        "complex": {
            "min": config.get("length_appropriateness.complex_turn_min"),
            "max": config.get("length_appropriateness.complex_turn_threshold"),
            "ideal": config.get("length_appropriateness.complex_turn_ideal"),
        },
    }
...
def compute_length_appropriateness_score(
...
def analyze_conversation_set_length(
    conversations: List[str], topic_docs: List[str]
) -> Dict:
    """
        ...
        detailed_results.append(
            {
                ...
                "is_within_range": turn_range["min_turns"]
                <= actual_turns
                <= turn_range["max_turns"],
            }
        )
    ...
    too_short = sum(
        1 for r in detailed_results if r["actual_turns"] < turn_range["min_turns"]
    )
    too_long = sum(
        1 for r in detailed_results if r["actual_turns"] > turn_range["max_turns"]
    )
