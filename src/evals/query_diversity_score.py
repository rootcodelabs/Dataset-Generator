...
class QueryDiversityAnalyzer:
    ...
    def __init__(self):
        ...
        self.semantic_similarity_threshold = self.config.get(
            "query_diversity.semantic_similarity_threshold"
        )
        self.min_queries_for_analysis = self.config.get(
            "query_diversity.min_queries_for_analysis"
        )
    ...
    def extract_user_queries(self, conversation: str) -> List[str]:
        ...
        if not queries:
            ...
            queries = [
                preprocess_text(s.strip()) for s in sentences if s.strip().endswith("?")
            ]
        ...
        try:
            ...
            vectorizer = TfidfVectorizer(
                ngram_range=ast.literal_eval(
                    self.config.get("query_diversity.ngram_range")
                ),
            ...
            lexical_diversity_score = (0.7 * unique_token_ratio) + (
                0.3 * ngram_diversity
            )
            ...
            return {"lexical_diversity_score": 0.0, "error": str(e)}
        ...
        try:
            ...
            for i in range(len(queries)):
                ...
                for j in range(i + 1, len(queries)):
                    if (
                        j not in query_cluster_map
                        and similarities[i, j] >= self.semantic_similarity_threshold
                    ):
                        cluster.append(j)
            ...
            largest_cluster_ratio = (
                largest_cluster_size / len(queries) if queries else 0
            )
            ...
            semantic_diversity_score = (
                (0.4 * cluster_ratio) + (0.4 * avg_dissimilarity) + (0.2 * size_balance)
            )
            ...
            return {"semantic_diversity_score": 0.0, "error": str(e)}

    def compute_query_diversity(
        self, conversations: List[str], topic_label: str = None
    ) -> Dict:
        """
        ...
        if (
            "lexical_diversity_score" in lexical_diversity
            and "semantic_diversity_score" in semantic_diversity
        ):
            overall_score = (
        ...
        first_query_semantic = (
            self.compute_semantic_diversity(first_queries)
            if first_queries
            else {"semantic_diversity_score": 0.0}
        )
        first_query_lexical = (
            self.compute_lexical_diversity(first_queries)
            if first_queries
            else {"lexical_diversity_score": 0.0}
        )
        ...
        if output_file:
            try:
                ...
                with open(output_file, "w", encoding="utf-8") as f:
                    json.dump(
                        results, f, ensure_ascii=False, indent=4, cls=NumpyEncoder
                    )
...
def compute_query_diversity_for_topic(
    conversations: List[str], topic_label: str = None
) -> float:
    """
...
def analyze_query_diversity_from_files(
    conversation_path: str, output_file: str = None, topic_label: str = None
) -> Dict:
    """
...
def analyze_multiple_topics(base_directory: str, output_directory: str = None) -> Dict:
    """
    analyze query diversity for multiple topics in subdirectories

    """
    topic_dirs = [
        d for d in glob.glob(os.path.join(base_directory, "*")) if os.path.isdir(d)
    ]
    ...
    for topic_dir in topic_dirs:
        ...
        if output_directory:
            ...
            output_file = os.path.join(
                output_directory, f"{topic_name}_query_diversity.json"
            )

        result = analyze_query_diversity_from_files(
            conversation_path=topic_dir, output_file=output_file, topic_label=topic_name
        )
    ...
    if all_results:
        summary = {
            ...
            "average_diversity_score": np.mean(
                [r["query_diversity_score"] for r in all_results]
            ),
            "average_first_query_diversity": np.mean(
                [r["first_query_diversity"] for r in all_results]
            ),
