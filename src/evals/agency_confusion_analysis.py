...
class AgencyConfusionAnalyzer:
    ...
    def __init__(self):
        ...
        self.confusion_threshold = self.config.get(
            "agency_confusion.confusion_threshold"
        )
        self.top_confusion_samples = self.config.get(
            "agency_confusion.top_confusion_samples"
        )
        self.stopwords = load_stopwords()

    def extract_user_queries(
        self, conversation: str, first_only: bool = False
    ) -> List[str]:
        """
        ...
        if not queries:
            ...
            question_sentences = [
                s.strip() for s in sentences if s.strip().endswith("?")
            ]
    ...
    def analyze_cross_agency_confusion(
        ...
        """
        if (
            not isinstance(agency_topics_conversations, dict)
            or len(agency_topics_conversations) < 2
        ):
        ...
        all_agencies_have_queries = all(
            count > 0 for count in agency_query_counts.values()
        )
        if not all_agencies_have_queries:
            empty_agencies = [
                agency for agency, count in agency_query_counts.items() if count == 0
            ]
            logger.error(f"No queries found for agencies: {', '.join(empty_agencies)}")
            return {
                "error": f"No queries found for agencies: {', '.join(empty_agencies)}"
            }
        ...
        for agency1, agency2 in agency_pairs:
            ...
            if topic_pair_metrics:
                most_confusable_topic_pair = max(
                    topic_pair_metrics.items(), key=lambda x: x[1]["confusion_rate"]
                )
            else:
        ...
        all_avg_similarities = [
            r["average_similarity"] for r in confusion_results.values()
        ]
        all_confusion_counts = [
            r["confusion_count"] for r in confusion_results.values()
        ]

        overall_results = {
            AGENCY_CONFUSION_KEYS["OVERALL_CONFUSION_RATE"]: np.mean(
                all_confusion_rates
            ),
            ...
            AGENCY_CONFUSION_KEYS["MOST_CONFUSABLE_PAIR"]: max(
                confusion_results.items(), key=lambda x: x[1]["confusion_rate"]
            )
    ...
    def analyze_topic_confusion_within_agency(
        self, topic_conversations: Dict[str, List[str]], agency_name: str = None
    ) -> Dict:
        """
        ...
        all_topics_have_queries = all(
            len(queries) > 0 for queries in topic_queries.values()
        )
        if not all_topics_have_queries:
            empty_topics = [
                topic for topic, queries in topic_queries.items() if not queries
            ]
    ...
    def generate_confusion_matrix_visualization(
        ...
        sns.heatmap(
            ...
            vmin=0,
            vmax=min(1.0, confusion_matrix.max() * 1.2),
            fmt=".3f",
        )
    ...
    def generate_topic_pair_heatmap(
        self, confusion_results: Dict, output_file: str = None
    ) -> None:
        """
        ...
        for (agency1, agency2), pair_result in agency_pair_results.items():
            for (topic1, topic2), topic_metrics in pair_result.get(
                "topic_pair_metrics", {}
            ).items():
...
def read_agency_topic_conversations(agency_dir: str) -> Dict[str, List[str]]:
    ...
    """
    topic_dirs = [
        d for d in glob.glob(os.path.join(agency_dir, "*")) if os.path.isdir(d)
    ]
    ...
    for topic_dir in topic_dirs:
        ...
        if conversations:
            ...
            logger.info(
                f"Read {len(conversations)} conversations for topic {topic_name}"
            )
        else:
            logger.warning(
                f"No conversations found for topic {topic_name} in {topic_dir}"
            )
...
def analyze_cross_agency_confusion(
    agency_dirs: Dict[str, str], output_dir: str = None
) -> Dict:
    """
    ...
    for agency_name, agency_dir in agency_dirs.items():
        ...
        if topic_conversations:
            ...
            total_conversations = sum(
                len(convs) for convs in topic_conversations.values()
            )
            total_topics = len(topic_conversations)
            logger.info(
                f"Read {total_conversations} conversations across {total_topics} topics for {agency_name}"
            )
        else:
            logger.warning(
                f"No topic conversations found for {agency_name} in {agency_dir}"
            )

    if len(agency_topic_conversations) < 2:
        logger.error(
            "Need at least two agencies with conversations to analyze confusion"
        )
    ...
    if output_dir and "error" not in results:
        ...
        analyzer.generate_confusion_matrix_visualization(
            results, viz_file, is_agency_level=True
        )
...
def analyze_within_agency_topic_confusion(
    agency_dir: str, agency_name: str = None, output_dir: str = None
) -> Dict:
    """
    ...
    if len(topic_conversations) < 2:
        logger.error(
            f"Need at least two topics with conversations to analyze confusion within {agency_name}"
        )
        return {
            "error": f"Need at least two topics with conversations within {agency_name}"
        }

    analyzer = AgencyConfusionAnalyzer()

    results = analyzer.analyze_topic_confusion_within_agency(
        topic_conversations, agency_name
    )
    ...
    if output_dir and "error" not in results:
        ...
        report_file = os.path.join(
            output_dir, f"{agency_name}_topic_confusion_report.json"
        )
        ...
        analyzer.generate_confusion_matrix_visualization(
            results, viz_file, is_agency_level=False
        )

        results["output_files"] = {"report": report_file, "visualization": viz_file}
