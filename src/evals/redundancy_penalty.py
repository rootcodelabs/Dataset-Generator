...
def compute_intra_conversation_redundancy(conversation: str) -> float:
    ...
    for i, j in itertools.combinations(range(n), 2):
        if similarity_matrix[i][j].item() >= config.get(
            "redundancy_penalty.intra_conversation_similarity_threshold"
        ):
            redundant_pairs += 1
        total_pairs += 1
...
def compute_inter_conversation_redundancy(conversations) -> Dict:
    ...
    for i, j in itertools.combinations(range(n), 2):
        sim = util.cos_sim(
            conversation_embeddings[i], conversation_embeddings[j]
        ).item()
        ...
        if sim >= config.get(
            "redundancy_penalty.inter_conversation_similarity_threshold"
        ):
...
def get_pairwise_comparison(conversation_files: List[str]):
    ...
    redundancy_info = compute_inter_conversation_redundancy(conversations)
...
def plot_similarity_heatmap(
    redundancy_info: Dict, output_file: str = "conversation_similarity.png"
) -> None:
    """
...
def generate_redundancy_report(
    topic_dir: str, output_file: str = "redundancy_report.json"
) -> None:
    ...
    results["similarity_threshold"] = config.get(
        "redundancy_penalty.inter_conversation_similarity_threshold"
    )
    ...
    for file_path in conversation_files:
        try:
            ...
            results["intra_redundancy"].append(
                {"file_path": file_path, "intra_redundancy": intra_redundancy}
            )
        except Exception as e:
            results["intra_redundancy"].append(
                {"file_path": file_path, "intra_redundancy": None}
            )
