...
from redundancy_penalty import compute_intra_conversation_redundancy, compute_inter_conversation_redundancy
...
class EvaluationPipeline:
    ...
    def __init__(self, config_path: str = 'eval_config.yaml'):
        ...
        self.file_structure = {
            ...
            "agency_subdir_pattern": self.config.get("file_structure.agency_subdir_pattern", ""),
            "topic_subdir_pattern": self.config.get("file_structure.topic_subdir_pattern", ""),
            
            # Conversation files configuration
            "conversation_dir": self.config.get("file_structure.conversation_dir", "conversations"),
            "conversation_file_pattern": self.config.get("file_structure.conversation_file_pattern", "conversation_*.txt"),
            "conversation_json_path": self.config.get("file_structure.conversation_json_path", ""),
            "conversation_json_key": self.config.get("file_structure.conversation_json_key", "conversations"),
            
            # Topic document files configuration
            "topic_dir": self.config.get("file_structure.topic_dir", "topics"),
            "topic_file_pattern": self.config.get("file_structure.topic_file_pattern", "*.txt"),
            "topic_json_path": self.config.get("file_structure.topic_json_path", ""),
            "topic_json_key": self.config.get("file_structure.topic_json_key", "content"),
        }
        
        # For backward compatibility
        self.topic_dir = self.config.get("general.topic_directory", self.file_structure["topic_dir"])
        self.conversation_dir = self.config.get("general.conversation_directory", self.file_structure["conversation_dir"])
        self.conversation_pattern = self.config.get("general.conversation_pattern", self.file_structure["conversation_file_pattern"])
    ...
    def _gather_from_directories(self) -> Dict[str, Dict[str, List[str]]]:
        ...
        if self.file_structure["agency_subdir_pattern"]:
            agency_dir_pattern = os.path.join(self.conversation_dir, self.file_structure["agency_subdir_pattern"])
            ...
            for agency_path in agency_dirs:
                ...
                if self.file_structure["topic_subdir_pattern"]:
                    topic_dir_pattern = os.path.join(agency_path, self.file_structure["topic_subdir_pattern"])
                    ...
                    for topic_path in topic_dirs:
                        ...
                        if conversations:
                            agency_topics_conversations[agency_name][topic_name] = conversations
                else:
                    # Assume topics are direct subdirectories
                    topic_dirs = [d for d in os.listdir(agency_path) 
                    ...
                    for topic in topic_dirs:
                        ...
                        if conversations:
                            agency_topics_conversations[agency_name][topic] = conversations
        else:
            # Assume direct agency directories
            agency_dirs = [d for d in os.listdir(self.conversation_dir) 
    ...
    def _gather_from_json(self) -> Dict[str, Dict[str, List[str]]]:
        ...
        try:
            ...
            if isinstance(content, dict):
                ...
                if "agency_key" in self.file_structure and self.file_structure["agency_key"]:
                    ...
                    for item_id, item in content.items():
                        ...
                        if conversation:
                            agency_topics_conversations[agency][topic].append(conversation)
                else:
                        ...
                        if isinstance(topics, dict):
                            ...
                            for topic, conversations in topics.items():
                                if isinstance(conversations, list):
                                    ...
                                    # Single conversation
                                    conversation = self._extract_conversation_from_json(conversations)
                                    if conversation:
                                        agency_topics_conversations[agency][topic] = [conversation]
                ...
                for item in content:
                    conversation = self._extract_conversation_from_json(item)
                    if conversation:
                        agency_topics_conversations[default_agency][default_topic].append(conversation)
    ...
    def _read_conversations_from_directory(self, directory: str) -> List[str]:
        ...
        """
        # First check for conversation text files
        conversation_files = glob.glob(os.path.join(directory, self.conversation_pattern))
        ...
        # If no text files found or explicitly instructed, check for JSON files
        if (not conversations or self.file_structure.get("check_json_fallback", True) or 
            self.file_structure.get("prefer_json", False)):
            ...
            if os.path.exists(conversations_json):
                try:
                    ...
                    # If data is a list, process each item as a conversation
                    if isinstance(data, list):
                        for item in data:
                            # Extract conversation from the JSON item
                            conversation = self._extract_conversation_from_json(item)
                            if conversation:
                                conversations.append(conversation)
                    else:
                        logger.warning(f"Unexpected JSON structure in {conversations_json}")
            ...
            if not conversations:
                ...
                for json_file in json_files:
                    ...
                    try:
                        ...
                        # Try to extract conversations based on common structures
                        if "conversations" in data:
                            if isinstance(data["conversations"], list):
                                for conv in data["conversations"]:
                                    conversation = self._extract_conversation_from_json(conv)
                                    if conversation:
                                        conversations.append(conversation)
    ...
    def _gather_topic_documents_from_directories(self) -> Dict[str, Dict[str, str]]:
        ...
        if self.file_structure.get("simplified_topic_structure", False):
            agency_dirs = [d for d in os.listdir(self.topic_dir) 
            ...
            for agency in agency_dirs:
                ...
                topic_files = glob.glob(os.path.join(agency_path, self.file_structure["topic_file_pattern"]))
        ...
        if self.file_structure["agency_subdir_pattern"]:
            ...
            if "output_" in agency_pattern:
                ...
                directories = [d for d in os.listdir(self.topic_dir) 
                              if os.path.isdir(os.path.join(self.topic_dir, d)) and d.startswith("output_")]
                ...
                for output_dir in directories:
                    ...
                    if agency_match:
                        ...
                        topic_files = glob.glob(os.path.join(self.topic_dir, output_dir, self.file_structure["topic_file_pattern"]))
                        
                        for topic_file in topic_files:
                            # Try to extract topic name from filename using patterns
                            topic_name = self._extract_topic_name_from_filename(topic_file, agency)
                ...
                for agency_path in agency_dirs:
                    ...
                    for topic_file in topic_files:
                        topic_name = self._extract_topic_name_from_filename(topic_file, agency_name)
                        topic_content = read_file(topic_file)
                        
                        if topic_content.strip():
                            agency_topic_documents[agency_name][topic_name] = topic_content
        else:
            # Look for output_* directories as a fallback for backward compatibility
            output_dirs = [d for d in os.listdir(self.topic_dir) 
                          if os.path.isdir(os.path.join(self.topic_dir, d)) and d.startswith("output_")]
            ...
            for output_dir in output_dirs:
                ...
                if agency_match:
                    ...
                    # Get all topic documents for this agency
                    topic_files = glob.glob(os.path.join(self.topic_dir, output_dir, "*.txt"))
        ...
        try:
            ...
            if isinstance(content, dict):
                ...
                if "agency_key" in self.file_structure and self.file_structure["agency_key"]:
                    ...
                    agency_key = self.file_structure["agency_key"]
                    topic_key = self.file_structure["topic_key"]
                    content_key = self.file_structure.get("topic_content_key", "content")
                        ...
                        if isinstance(topics, dict):
                            ...
                            for topic, topic_content in topics.items():
                                if isinstance(topic_content, str):
                                    agency_topic_documents[agency][topic] = topic_content
                                elif isinstance(topic_content, dict) and "content" in topic_content:
                                    agency_topic_documents[agency][topic] = topic_content["content"]
            elif isinstance(content, list):
                ...
                for i, item in enumerate(content):
                    if isinstance(item, str):
                        ...
                        if topic_content:
                            agency_topic_documents[default_agency][topic_name] = topic_content
    ...
    def evaluate_information_coverage(self, 
                                     conversation: str, 
                                     topic_doc: str) -> Dict:
        """
    ...
    def evaluate_length_appropriateness(self, 
                                       conversation: str, 
                                       topic_doc: str) -> Dict:
        """
    ...
    def evaluate_topic_consistency(self, 
                                  conversation: str, 
                                  topic_doc: str) -> Dict:
        """
        ...
        results = self.topic_consistency_evaluator.evaluate_conversation_topic_quality(conversation)
    ...
    def evaluate_intra_redundancy(self, 
        ...
        return {
            "intra_redundancy_score": float(score)
        }
    ...
    def evaluate_query_diversity(self, 
        ...
        return {
            "query_diversity_score": float(score)
        }
    
    def evaluate_inter_redundancy(self, 
    ...
    def evaluate_topic_coverage(self, 
                              conversations: List[str], 
                              topic_doc: str) -> Dict:
        """
        ...
        """
        results = self.topic_coverage_analyzer.analyze_topic_coverage(topic_doc, conversations)
    ...
    def evaluate_qualitative(self, 
        ...
        """
        result = self.qualitative_evaluator.evaluate_conversation(conversation, "overall_quality")
    ...
    def evaluate_topic_agency_fit(self, 
                                conversation: str,
                                topic_name: str,
                                topic_doc: str,
                                agency_name: str) -> Dict:
        """
        ...
        """
        agency_description = self.topic_agency_fit_evaluator.read_agency_description(agency_name)
        ...
        return {
            ...
            "topic_fit_score": float(result.get("numerical_scores", {}).get("topic_fit", 0) / 5.0),
            "agency_fit_score": float(result.get("numerical_scores", {}).get("agency_fit", 0) / 5.0)
        }
    
    def evaluate_agency_confusion(self, 
        ...
        """
        results = self.agency_confusion_analyzer.analyze_cross_agency_confusion(agency_topics_conversations)
    ...
    def evaluate_conversation(self, 
        ...
        if self.config.get("length_appropriateness.enabled"):
            results.update(self.evaluate_length_appropriateness(conversation, topic_doc))
        ...
        if self.config.get("topic_agency_fit.enabled", False):
            results.update(self.evaluate_topic_agency_fit(conversation, topic_name, topic_doc, agency_name))
    ...
    def run_evaluation(self) -> Dict:
        ...
        if self.config.get("agency_confusion.enabled") and len(agency_topics_conversations) > 1:
            agency_confusion_results = self.evaluate_agency_confusion(agency_topics_conversations)
        ...
        for agency_name, topics_conversations in agency_topics_conversations.items():
            ...
            for topic_name, conversations in topics_conversations.items():
                if agency_name not in agency_topic_documents or topic_name not in agency_topic_documents[agency_name]:
                    logger.warning(f"No topic document found for {agency_name}/{topic_name}")
                    continue
                ...
                for i, conversation in enumerate(conversations):
                    ...
                    self.results["by_conversation"][f"{agency_name}/{topic_name}/{conv_id}"] = conv_results
                ...
                # Save topic results
                self.results["by_agency"][agency_name]["by_topic"][topic_name] = topic_results
                self.results["by_topic"][f"{agency_name}/{topic_name}"] = topic_results["overall"]
            
            # Compute average metrics for the agency
            self.results["by_agency"][agency_name]["overall"] = self.average_results(agency_conversation_results)
        
        # Compute overall averages across all agencies and topics
        all_topic_results = list(self.results["by_topic"].values())
        self.results["overall"]["average_scores"] = self.average_results(all_topic_results)
    ...
    def generate_summary_report(self) -> None:
        ...
        if topic_data:
            ...
            self.generate_score_heatmap(topic_df, "topic", 
        ...
        if agency_data:
            ...
            self.generate_score_heatmap(agency_df, "agency", 
        ...
        if overall_scores:
            ...
            overall_df = pd.DataFrame([{"metric": k, "score": v} 
                                      for k, v in overall_scores.items()])
            overall_df.to_csv(overall_file, index=False)
            
            # Generate overall scores bar chart
            self.generate_overall_scores_chart(overall_scores, 
                                             os.path.join(self.output_dir, "overall_scores.png"))
    
    def generate_score_heatmap(self, 
                              df: pd.DataFrame, 
                              id_column: str,
        ...
        column_labels = [col.replace('_score', '').replace('_', ' ').title() 
    ...
    def generate_overall_scores_chart(self, 
        ...
        score_items = [(k.replace('_score', '').replace('_', ' ').title(), v) 
                      for k, v in scores.items() if k.endswith('_score')]
        ...
        for bar in bars:
            width = bar.get_width()
            plt.text(max(width + 0.01, 0.05), bar.get_y() + bar.get_height()/2,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Run the conversation evaluation pipeline.')
    ...
    logger.info(f"Evaluation completed successfully. Results saved to {pipeline.output_dir}")
