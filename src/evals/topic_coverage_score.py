...
class TopicCoverageAnalyzer:
    ...
    def __init__(self):
        ...
        self.clustering_threshold = self.config.get(
            "topic_coverage.clustering_threshold"
        )
    ...
    def _evaluate_conversation_topic_coverage(
        self, conversations: List[str], document_topics: List[Dict]
    ) -> Dict:
        """
        ...
        conversation_embeddings = self.embedding_model.encode(
        ...
        for topic in document_topics:
            ...
            similarities = cosine_similarity([topic_centroid], conversation_embeddings)[
                0
            ]
        ...
        covered_topics = sum(
            1 for t in topic_coverage.values() if bool(t["is_covered"])
        )
        coverage_percentage = (
            str((covered_topics / total_topics) * 100) if total_topics > 0 else "0"
        )

        conversation_coverage = []
        for i, (conv, embedding) in enumerate(
            zip(conversations, conversation_embeddings)
        ):
            covered_topics = []
            for topic in document_topics:
                topic_id = topic["topic_id"]
    ...
    def analyze_topic_coverage(self, document: str, conversations: List[str]) -> Dict:
        """
        ...
        coverage_results = self._evaluate_conversation_topic_coverage(
            conversations, self.document_topics
        )
    ...
    def generate_coverage_report(
        ...
        if not conversation_files:
            conversation_files = [
                f"conversation_{i + 1}.txt" for i in range(len(conversations))
            ]
    ...
    def _generate_markdown_report(
        self, coverage_results: Dict, output_file: str
    ) -> None:
    ...
    def _generate_coverage_heatmap(
        ...
        try:
            ...
            conversation_ids = [
                i for i in range(len(coverage_results["conversation_coverage"]))
            ]

            coverage_matrix = np.zeros((len(conversation_ids), len(topic_ids)))

            for conv_idx, conv_coverage in enumerate(
                coverage_results["conversation_coverage"]
            ):
            ...
            plt.figure(
                figsize=(
                    max(8, len(topic_ids) * 0.5),
                    max(6, len(conversation_ids) * 0.4),
                )
            )
            ...
            for topic_id in topic_ids:
                topic = next(
                    (t for t in self.document_topics if t["topic_id"] == topic_id), None
                )
                if topic:
...
def analyze_topic_coverage_from_files(
    conversation_path: str, topic_docs_path: str, output_dir: str = None
) -> Dict:
    """
    ...
    return analyzer.generate_coverage_report(
        document, conversations, filenames, output_dir
    )
