...
async def generate_bulk_dataset(
    ...
    try:
        logger.info(
            f"Received bulk generation request: {request.dataset_structure_name}, {request.prompt_template_name}"
        )
        ...
        for source in data_sources:
            ...
            if traversal_strategy == "institutional":
                ...
                output_base_path = (
                    f"{output_dir}/{structure_name}/{institution}/{topic}_{timestamp}"
                )
            else:
                # For other traversal strategies, use relative path
                rel_path = source.metadata.get("relative_path", source.name)
                output_base_path = (
                    f"{output_dir}/{structure_name}/{rel_path}_{timestamp}"
                )
