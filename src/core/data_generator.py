...
class DataGenerator:
    ...
    def generate(
        ...
        if "language" in parameters:
            if not isinstance(parameters["language"], str) or len(parameters["language"]) > 10:
        ...
        if output_base_path and ".." in output_base_path:
            logger.warning(f"Path traversal attempt detected in output_base_path: {output_base_path}")
            output_base_path = output_base_path.replace("..", "")

        num_examples = num_examples or self.generation_defaults.get("default_num_examples", 5)
