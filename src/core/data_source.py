...
class DataSourceFilter:
    ...
    def __init__(self, 
        ...
        """
        self.extensions = [ext.lstrip('.').lower() for ext in extensions] if extensions else None
        self.pattern_objects = [re.compile(p, re.IGNORECASE) for p in patterns] if patterns else None
...
class DataSourceTraversalStrategy:
    ...
    def traverse(self, base_path: str, filter_obj: Optional[DataSourceFilter] = None) -> Iterator[DataSource]:
        """
...
class DataSourceManager:
    ...
    def create_filter_from_config(self, filter_config: Dict[str, Any]) -> DataSourceFilter:
        """
    ...
    def load_sources(self, 
                    ...
                    filter_config: Dict[str, Any] = None) -> List[DataSource]:
        """
        ...
        logger.info(f"Loaded {len(sources)} data sources from {base_path} using '{strategy_name}' strategy")
